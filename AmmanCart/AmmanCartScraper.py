...
SKU/search-term found in an input Excel file, then writes the results
into an output Excel file.

Launcher-compatible usage:
    python zananiri_scraper.py --in input.xlsx --sku-col "Variant SKU" --out output.xlsx

Standalone usage still works the same way (just pass the flags explicitly).
...
# =========================
# CORE FUNCTIONS (unchanged scraping logic)
# =========================
...
def load_input_dataframe(path: str, sku_column: str) -> pd.DataFrame:
    """
    ...
    `sku_column` is the column name selected by the user in the launcher
    (or passed via --sku-col on the CLI).
    """
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Zananiri JO scraper - scrapes product descriptions by SKU"
    )
    parser.add_argument(
        "--in",
        dest="input_file",
    ...
    parser.add_argument(
        ...
        dest="sku_column",
        required=True,
        help="Column name containing SKUs/search terms",
    )
    parser.add_argument(
        "--out",
        dest="output_file",
        required=True,
        help="Path to write the output Excel file",
    )
    parser.add_argument(
        "--headful",
        action="store_true",
...
def main() -> None:
    ...
    try:
        df = load_input_dataframe(input_file, sku_column)
